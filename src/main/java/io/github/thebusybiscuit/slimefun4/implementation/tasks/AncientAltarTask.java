...
public class AncientAltarTask implements Runnable {
    ...
    private void idle() {
        ...
        for (Location loc : particleLocations) {
            ...
            dropLocation.getWorld().spawnParticle(Particle.CRIT, loc, 8, 0.3F, 0.2F, 0.3F);
        }
    }
    ...
    private void checkPedestal(@Nonnull Block pedestal) {
        ...
        if (!item.isPresent() || positionLock.remove(item.get()) == null) {
            ...
            dropLocation.getWorld().spawnParticle(Particle.CRIT, pedestal.getLocation().add(0.5, 1.5, 0.5), 8, 0.3F, 0.2F, 0.3F);
